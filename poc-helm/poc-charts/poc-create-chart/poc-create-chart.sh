...
function main() {
  ...
  print_box "CREATE CHART" \
    "" \
    " - Proof of concept on creating a chart"
  ...
  export POD_NAME=$(kubectl get pods --namespace poc-charts -l "$LABELS" -o jsonpath="{.items[0].metadata.name}")
  export CONTAINER_PORT=$(kubectl get pod --namespace poc-charts $POD_NAME -o jsonpath="{.spec.containers[0].ports[0].containerPort}")
