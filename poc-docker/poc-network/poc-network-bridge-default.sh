...
function main {
  ...
  docker::networkInspect $DEFAULT_NETWORK
  ...
  CONTAINER1_IP_ADDRESS=$(docker::getIpAddressFromContainer ${CONTAINER1_NAME})
  echo ${CONTAINER1_IP_ADDRESS}

  CONTAINER2_IP_ADDRESS=$(docker::getIpAddressFromContainer ${CONTAINER2_NAME})
  echo ${CONTAINER2_IP_ADDRESS}
  checkInteractiveMode

  print_info "Check connection from ${CONTAINER1_NAME} to ${CONTAINER2_NAME} by ip address"
  docker::execContainerPing ${CONTAINER1_NAME} ${CONTAINER2_IP_ADDRESS}
  if [ $? -ne 0 ]; then
  ...
  docker::execContainerPing ${CONTAINER2_NAME} ${CONTAINER1_IP_ADDRESS}
  if [ $? -ne 0 ]; then
    print_error "Poc completed with failure"
    exit 1
  fi
  ...
  docker::execContainerPing ${CONTAINER2_NAME} ${CONTAINER1_NAME}
  if [ $? -eq 0 ]; then
    print_error "Poc completed with failure"
    exit 1
  fi
